...
void jacobiIteration(double *h, double *g, ll matrixSize, ll iterations)
{
    ...
    dim3 dimBlock(32, 32);
    dim3 dimGrid(matrixSize/32, matrixSize/32);
    for (ll iterator = 0; iterator < iterations; iterator++)
    {

   	calculateGValue<<<dimGrid, dimBlock>>>( hd, gd, matrixSize);
    	HANDLE_ERROR( cudaMemcpy(g,gd,size,cudaMemcpyDeviceToHost));
    	cudaDeviceSynchronize();
    	HANDLE_ERROR( cudaMemcpy(hd, g, size, cudaMemcpyHostToDevice));
    	cudaDeviceSynchronize();
    }
