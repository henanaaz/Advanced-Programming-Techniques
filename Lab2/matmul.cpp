 ...
 * pragma @fPointer takes the fstream for reading the values
 * pragma @rows represents the number of rows
 * pragma @cols represents the number of columns
  **/
 ...
 * pragma @matrix is input vector to take matrix on which transpose will be performed
 **/
 ...
 * pragma @fileName string is to specify the name of input file stream
 * **/
...
#pragma omp parallel for private(k) shared(matrix3) collapse(2)
	for (i = 0; i < matrix1.size(); i++) 
	{
		for (j = 0; j < matrix2.size(); j++) 
		{
			vector<ld>::iterator row = matrix1[i].begin();
			vector<ld>::iterator col = matrix2[j].begin();

			long double sum = 0.0;

			//Calculate the value of one entry in matrix in the sum, for each rows and columns multiplied
			for (k = 0; k < matrix1.size(); k++) {
				sum += row[k] * col[k];
			}
